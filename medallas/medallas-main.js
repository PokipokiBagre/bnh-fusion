...
import { renderCatalogo, renderGrafo, renderPersonaje, renderDetalleMedalla, renderFormMedalla, _htmlReqRow, _htmlCondRow, toast, mountNewTagAC } from './medallas-ui.js';
...
window.onload = async () => {
    ...
    try {
        // Cargar tags canónicos y datos en paralelo
        await Promise.all([
            initTags(),
            cargarTodo(),
        ]);
...
function _exponerGlobales() {
    ...
    window._medGrafoReset = () => { buildGraph(); };
    ...
    // ── Formulario: añadir filas dinámicas ──
    window._medAddReq = () => {
        ...
        // Montar AC en el nuevo input tras inserción en DOM
