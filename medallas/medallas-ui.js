        ...
        wrap.innerHTML = `
            <div class="grafo-wrap" style="height:calc(100vh - 160px);">
                ...
                <div class="grafo-legend">
                    ...
                    <div style="color:#aaa;margin-top:4px;">━━ Requisito base</div>
...
export function renderPersonaje() {
    ...
    if (!pj) {
        ...
        // Agrupar medallas por tag del PJ
...
export function renderDetalleMedalla(m, pjNombre = null) {
    ...
    const reqsHtml = (m.requisitos_base||[]).map(r =>
        `<div style="font-size:0.82em;padding:5px 0;border-bottom:1px solid var(--gray-100);">
            ...
            <span style="margin-left:6px;color:var(--gray-700);">mín. <b>${r.pts_minimos} PT</b> para equipar</span>
        </div>`
    ).join('') || '<p style="color:var(--gray-400);font-size:0.82em;">Sin requisitos base.</p>';
        ...
        <div style="position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:500;display:flex;align-items:flex-start;justify-content:center;padding:40px 16px;overflow-y:auto;">
            <div style="background:white;border-radius:var(--radius-lg);max-width:620px;width:100%;box-shadow:var(--shadow-lg);overflow:hidden;">
                ...
                <div style="padding:20px;display:flex;flex-direction:column;gap:14px;">
                    ...
                    <div>
                        <div class="form-label">Requisitos para equipar</div>
                        ${reqsHtml}
                    </div>
...
// ── Modal creador/editor de medalla (OP) ──────────────────────
export function renderFormMedalla(m = null) {
    const isEdit = !!m;
    const tagsExistentes = [...new Set(medallas.flatMap(g => g.tags||[]))].sort();
    const tagOptions = tagsExistentes.map(t => `<option value="${t}">${t}</option>`).join('');
            ...
            <div style="background:white;border-radius:var(--radius-lg);max-width:700px;width:100%;box-shadow:var(--shadow-lg);overflow:hidden;">
                ...
                <div class="modal-body">
                    ...
                    <div>
                        ...
                        <div style="font-size:0.75em;color:var(--gray-500);margin-bottom:6px;">
                            El PJ debe tener el tag Y los PT mínimos para poder equipar esta medalla.
                        </div>
                        <div id="fm-reqs">
                    ...
                    <div>
                        ...
                        <div style="font-size:0.75em;color:var(--gray-500);margin-bottom:6px;">
                            Se activan si el PJ tiene el tag con los PT requeridos al equipar.
                        </div>
                        <div id="fm-conds">
...
export function _htmlCondRow(c = {}, idx) {
    ...
    return `<div class="cond-row" style="flex-direction:column;align-items:stretch;" id="cond-row-${idx}">
        ...
        <textarea class="inp" rows="2" placeholder="Efecto si se cumple la condición…" id="cond-efecto-${idx}" style="margin-top:6px;">${_esc(c.efecto||'')}</textarea>
    </div>`;
}
