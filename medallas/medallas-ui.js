...
export function renderGrafo() {
    ...
    // Construir índice de tags → cuántas medallas tienen
    const tagConteo = {};
    medallas.filter(m => !m.propuesta || medallaState.esAdmin).forEach(m => {
        mTags(m).forEach(t => { tagConteo[t] = (tagConteo[t]||0) + 1; });
    });
    // Ordenar por cantidad de medallas desc
    let todosLosTags = Object.entries(tagConteo)
        .sort((a,b) => b[1]-a[1])
        .map(([tag, cnt]) => ({ tag, cnt }));

    // Buscador filtra y mueve al principio
        ...
        <div style="display:flex;flex-direction:column;gap:10px;">
            ...
            <div style="background:white;border:1.5px solid #dee2e6;border-radius:12px;padding:14px;">
                <div style="display:flex;gap:8px;align-items:center;margin-bottom:10px;flex-wrap:wrap;">
                    <b style="font-size:0.85em;color:var(--gray-700);">Selecciona tags para ver en el grafo</b>
                    ...
                    <button class="btn btn-sm btn-outline" onclick="window._medGrafoReset()">🎯 Centrar</button>
                </div>
            ...
            <!-- Leyenda -->
            <div style="display:flex;gap:16px;font-size:0.78em;color:#888;padding:0 4px;align-items:center;">
                <span>
                    <svg width="14" height="14" style="vertical-align:middle;margin-right:4px;">
                        <rect x="2" y="2" width="10" height="10" fill="rgba(243,156,18,0.3)" stroke="#f39c12" stroke-width="1.5" transform="rotate(45 7 7)"/>
                    </svg>
                    Tag
                </span>
                <span>
                    <svg width="14" height="14" style="vertical-align:middle;margin-right:4px;">
                        <polygon points="7,1 13,13 1,13" fill="rgba(0,180,216,0.3)" stroke="#00b4d8" stroke-width="1.5"/>
                    </svg>
                    Medalla
                </span>
                <span>
                    <svg width="14" height="14" style="vertical-align:middle;margin-right:4px;">
                        <polygon points="7,1 13,13 1,13" fill="rgba(243,156,18,0.3)" stroke="#f39c12" stroke-width="1.5"/>
                    </svg>
                    Propuesta
                </span>
                <span style="color:#888;">Click en nodo = detalle · Scroll = zoom · Arrastrar = mover</span>
            </div>

            <!-- Canvas -->
            <div style="position:relative;background:#0d1117;border-radius:12px;overflow:hidden;">
                <canvas id="medallas-canvas" style="display:block;"></canvas>
                ${!selTags.length ? `<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#aaa;font-size:0.9em;pointer-events:none;">← Selecciona un tag para ver su grafo</div>` : ''}
            </div>
        </div>`;

    // Iniciar o reusar canvas
    setTimeout(() => {
